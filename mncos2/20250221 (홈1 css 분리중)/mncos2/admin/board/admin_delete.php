...
if ($id > 0) {
    try {
        ...
        $stmt->execute([$id]);
        echo "<script>alert('삭제되었습니다.'); location.href='/mncos2/admin/board/inquiry.php';</script>";
    } catch (Exception $e) {
        echo "<script>alert('오류가 발생했습니다.'); history.back();</script>";
    }
} else {
    echo "<script>alert('잘못된 접근입니다.'); location.href='/mncos2/admin/board/inquiry.php';</script>";
}
?>