...
function autosave() {
    $('form#fwrite').each(function () {
        ...
        if (save_wr_subject != this.wr_subject.value || save_wr_content != this.wr_content.value) {
            $.ajax({
                url: g5_community_url + '/ajax.autosave.php',
                data: {
                    uid: this.uid.value,
...
$(function () {
    ...
    $('#btn_autosave').click(function () {
        if ($('#autosave_pop').is(':hidden')) {
            $.get(
                g5_community_url + '/ajax.autosavelist.php',
                function (data) {
                    //alert(data);
                    ...
                    if ($(data).find('list').find('item').length > 0) {
                        $(data)
                            ...
                            .each(function (i) {
                                ...
                                $('#autosave_pop ul')
                                    .append(
                                        '<li><a href="#none" class="autosave_load">' +
                                            subject +
                                            '</a><span>' +
                                            datetime +
                                            ' <button type="button" class="autosave_del">삭제</button></span></li>'
                                    )
                                    .find('li:eq(' + i + ')')
    ...
    $(document).on('click', '.autosave_load', function () {
        ...
        $.get(
            g5_community_url + '/ajax.autosaveload.php',
            { as_id: as_id },
            function (data) {
    ...
    $(document).on('click', '.autosave_del', function () {
        var $li = $(this).parents('li');
        var as_id = $li.data('as_id');
        $.get(g5_community_url + '/ajax.autosavedel.php', { as_id: as_id }, function (data) {
            if (data == -1) {
