...
class G5_URI
{
    ...
    public function url_clean($string_url, $add_qry = '')
    {
        ...
        if (strpos($string_url, G5_COMMUNITY_URL) === false || !in_array($page_name, $array_page_names)) {   //게시판이 아니면 리턴
            return $string_url;
        }
        ...
        if (isset($url['host'])) {
            ...
            $array_file_paths = run_replace('url_clean_page_paths', array('/' . G5_COMMUNITY_DIR . '/notice.php', '/' . G5_COMMUNITY_DIR . '/faq.php', '/' . G5_COMMUNITY_DIR . '/gallery.php', '/portfolio.php', '/' . G5_COMMUNITY_DIR . '/write.php'));
