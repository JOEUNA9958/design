...
function get_paging($write_pages, $cur_page, $total_page, $url, $add = "")
{
    ...
    if ($total_page > 1) {
        for ($k = $start_page; $k <= $end_page; $k++) {
            if ($cur_page != $k)
                $str .= '<a href="' . $url . $k . $add . '" class="page-num">' . $k . '</a>' . PHP_EOL;
            else
                $str .= '<a href="javascript:void(0);" class="page-num current">' . $k . '</a>' . PHP_EOL;
        }
    }
...
function alert($msg = '', $url = '', $error = true, $post = false)
{
    ...
    include_once(G5_COMMUNITY_PATH . '/alert.php');
    exit;
}
...
function alert_close($msg, $error = true)
{
    ...
    include_once(G5_COMMUNITY_PATH . '/alert_close.php');
    exit;
}
...
function confirm($msg, $url1 = '', $url2 = '', $url3 = '')
{
    ...
    include_once(G5_COMMUNITY_PATH . '/confirm.php');
    exit;
}
...
function get_list($write_row, $board, $subject_len = 40)
{
    ...
    $g5_object->set('community', $write_row['wr_id'], $write_row, $board['bo_table']);
    ...
    for ($i = 1; $i <= G5_LINK_COUNT; $i++) {
        ...
        $list['link_href'][$i] = G5_COMMUNITY_URL . '/link.php?bo_table=' . $board['bo_table'] . '&amp;wr_id=' . $list['wr_id'] . '&amp;no=' . $i . $qstr;
...
// get_list 의 alias
function get_view($write_row, $board): mixed
{
    return get_list($write_row, $board, 255);
}
    ...
    for ($i = 0; $i < count($s); $i++) {
        ...
        for ($k = 0; $k < count($field); $k++) { // 필드의 수만큼 다중 필드 검색 가능 (필드1+필드2...)
            ...
            switch ($field[$k]) {
                ...
                default:
                    if ($field[$k] === 'wr_content') {
                        // 완전히 제거할 태그 패턴 (img, iframe 등)
                        $strip_pattern = array(
                            '<img[^>]+>',              // img 태그
                            '<iframe[^>]+>.*?</iframe>',  // iframe 태그
                            '<video[^>]+>.*?</video>',    // video 태그
                            '<audio[^>]+>.*?</audio>',    // audio 태그
                            '<source[^>]+>',           // source 태그
                            '<input[^>]+>',            // input 태그
                            '<br[^>]*>',              // br 태그
                            '<hr[^>]*>'               // hr 태그
                        ...
                        // 여는 태그와 닫는 태그만 제거할 패턴
                        $content_tags_pattern = array(
                            '</?p[^>]*>',             // p 태그
                            '</?div[^>]*>',           // div 태그
                            '</?span[^>]*>',          // span 태그
                            '</?a[^>]*>',             // a 태그
                        ...
                        $all_patterns = array_merge($strip_pattern, $content_tags_pattern);
                        $strip_tags = implode('|', $all_patterns);

                        if (preg_match("/[a-zA-Z]/", $search_str)) {
                            $str .= "INSTR(LOWER(REGEXP_REPLACE($field[$k], '$strip_tags', '')), LOWER('$search_str'))";
                        } else {
                            $str .= "INSTR(REGEXP_REPLACE($field[$k], '$strip_tags', ''), '$search_str')";
                        }
                    } else {
                        if (preg_match("/[a-zA-Z]/", $search_str))
...
function get_write($write_table, $wr_id, $is_cache = false)
{
    ...
    $write = $g5_object->get('community', $wr_id, $wr_bo_table);

    if (!$write || $is_cache == false) {
        $sql = " select * from {$write_table} where wr_id = '{$wr_id}' ";
        $write = sql_fetch($sql);

        $g5_object->set('community', $wr_id, $write, $wr_bo_table);
    }
...
function view_file_link($file, $width, $height, $content = '')
{
    ...
    if (preg_match("/\.({$config['cf_image_extension']})$/i", $file) && isset($board['bo_table'])) {
        $attr_href = run_replace('thumb_view_image_href', G5_COMMUNITY_URL . '/view_image.php?bo_table=' . $board['bo_table'] . '&amp;fn=' . urlencode($file), $file, $board['bo_table'], $width, $height, $content);
...
function member_delete($mb_id)
{
    ...
    $sql = " select mb_name, mb_nick, mb_ip, mb_recommend, mb_level from {$g5['member_table']} where mb_id= '" . $mb_id . "' ";
    $mb = sql_fetch($sql);

    // 회원자료는 정보만 없앤 후 아이디는 보관하여 다른 사람이 사용하지 못하도록 함 : 061025
    $sql = " update {$g5['member_table']} set mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_hp = '', mb_birth = '', mb_sex = '', mb_signature = '', mb_certify = '', mb_adult = 0, mb_dupinfo = '' where mb_id = '{$mb_id}' ";
...
function decrypt_string($ciphertext, $key)
{
    ...
    list($ciphertext, $iv) = explode(':', $ciphertext_dec, 2);
