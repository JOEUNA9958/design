$(document).ready(function(){
    ...
    let tab_name //find 의 클릭한 tab의 이름
     ...
     * 1. 스크롤 내린 상태에서 header에 마우스를 올렸다가 내림 (fixed 사라지면 안됌 )
    /**/
    ...
    $('header').on('mouseleave' , function(){
        ...
        스크롤 된 값이 0이거나 0보다 작을때만 삭제 **/
        if(scrolling <= 0){
            if($('header').hasClass('sch_open') == false){ // header에 sch_open에 없을때
    ...
    function scroll_chk(){
        ...
        if(scrolling > 0){ //스크롤을 내렸을때
            ...
            /** 검색이 열려있는 상태에서는 class삭제 안함 --- header에 sch_open 클래스가 있으면 열린 상태 **/
    ...
    /********** 검색창 열기 (시작) ********
     * header .tnb .search .search_open 클릭하면 header에 sch_open 추가
     * header .tnb .search .search_wrap .search_close  를 클릭하면 header에  sch_open 삭제
     * header.sch_open::after 한테는 클릭이벤트가 발생안함 / before, after 둘다 안됌 
     * **/
    ...
    // 모바일 2차 메뉴 열기 시작
    /*
    * li를 클릭할것이냐 a를 클릭할것이냐 -> a를 클릭
    * --> 2차 메뉴가 있는 1차 메뉴는 하위메뉴를 여는 기능을 갖음 (자기 자신의 href는 작동안함)
    * --> 2차 메뉴를 가지고 있는 1차 메뉴인지 아닌지 구분
    * header .gnb .gnb_wrap ul.depth1 > li:has(ul.depth2) > a
    * 1차 메뉴 li에 open 이라는 클래스를 추가 
    *
    * 1. 클릭한 메뉴만 열리고 다른 메뉴는 모두 닫기
    * 2.이미 열려있는 메뉴를 다시 클릭하면 닫기
    * 메뉴를 클릭했을때 이미 열린 메뉴 (나만닫기) 또는  닫혀 메뉴 (나머지 다 닫고, 나만 열기)
    */ 
    $('header .gnb .gnb_wrap ul.depth1 > li:has(ul.depth2) > a').on('click', function(e){
        if( device_status == 'mobile'){ // 모바일 일때만 
            e.preventDefault(); //href 를 작동안하게 막는 코드 
        }
     ...
     * 버튼을 누른 후 하는 일 
     * li에 active 클래스 추가, li button 에 title="선택됨 입력
     * li에 data-tab 의 값을 가져와서 tab_content .tab_panel 중에서 data-tab 이 같은 값인 요소
     * 
     * 
     * **/
