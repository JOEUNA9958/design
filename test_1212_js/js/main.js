...
let abc = '1'
abc = '만세' /* =은 할당연산자 abc = '만세'라고 넣어주면 이전 값을 지우고 나만 들어감 */
var bbb = '2'
console.log(abc)
console.log('bbb') //변수를 따옴표로 감싸면 그건 그냥 글자가 됨
// 브라우저의 콘솔창에 로그 메시지를 출력해달라  abc라고 하는 변수에 담겨있는 메시지 
...
sum = sum - num1 //현재 sum 값에서 num1의 겂을 빼줌
console.log(sum)
if(sum > 20) { //만약 sum이라는 변수의 값이 20보다 크다면
    console.log('sum은 20보다 크다') //위 조건이 맞으면 실행, 조건이 안맞으면 아예 실행x
}
...
/*
    html에서 js파일을 먼저 호출했고 다음에 html요소가 쓰임 (순서가중요)
    그럼 js에서 html요소를 불러서 스타일을 줄 수가 없음 (명령을 내릴 수 없음)
    --->해결방법은 js를 html아래로 내리는 것인데 이는 추천하지 않음 !!
*/
